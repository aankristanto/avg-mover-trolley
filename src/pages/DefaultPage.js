...
const DefaultPage = () => {
    ...
    const getListLogByStation = async (id, isLoading) => {
        if (isLoading) setLoadingMain(true)
        try {
            ...
            if (isLoading) setLoadingMain(false)
        } catch (error) {
            if (isLoading) setLoadingMain(false)
            setFailedPage(true)
            console.error("Error fetching station list:", error);
    ...
    const handleConfirmState = () => {
        ...
        getListLogByStation(SelectedStation.STATION, true)
        setFirstSetup(false)
    }
    ...
    useEffect(() => {
        ...
        if (strg) {
            ...
            getListLogByStation(data.STATION, true)
